...
namespace FastyBird\HomeKitConnector\Crypto;
...
final class Poly1305
{
	...
	/** @var SplFixedArray<int|SplFixedArray<int>> */
	private SplFixedArray $ctx;
	 ...
	 * @return int
	 */
	private function load(array $x, int $i = 0): int
	{
		return $x[$i] | ($x[1 + $i] << 8) | ($x[2 + $i] << 16) | ($x[3 + $i] << 24);
	}

	private function store($x, $i = 0, $u)
	{
		$x[$i] = $u & 0xff;
		$u >>= 8;
		$x[1 + $i] = $u & 0xff;
	...
	private function blocks($data, $offset = 0, $length)
	{
	...
	public function finish()
	{
		...
		$f = $h0 + $this->ctx[2][0];
		$h0 = $f;
		$f = $h1 + $this->ctx[2][1] + ($f >> 32);
		$h1 = $f;
		$f = $h2 + $this->ctx[2][2] + ($f >> 32);
		$h2 = $f;
		$f = $h3 + $this->ctx[2][3] + ($f >> 32);
		$h3 = $f;
