...
final class Chacha20
{
	...
	private function load32($x, $i)
	{
		return $x[$i] | ($x[$i + 1] << 8) | ($x[$i + 2] << 16) | ($x[$i + 3] << 24);
	}

	private function store32($x, $i, $u)
	{
		$x[$i] = $u & 0xff;
		$u >>= 8;
		$x[$i + 1] = $u & 0xff;
	...
	private function rotl32($v, $c)
	{
		return (($v << $c) & 0xffffffff) | ($v >> (32 - $c));
	}

	private function round($x, $a, $b, $c, $d)
	{
		$x[$a] = $this->plus($x[$a], $x[$b]);
		$x[$d] = $this->rotl32($x[$d] ^ $x[$a], 16);
		$x[$c] = $this->plus($x[$c], $x[$d]);
